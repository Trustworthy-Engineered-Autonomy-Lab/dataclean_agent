...
- Adaptive mode: choose actions from observable evidence and provide concise rationales. For an applied partition, include distribution_shape, prior_assumptions_and_uncertainty, candidate_comparison, main_risk, and confidence in the evidence object. Follow the partition prior's BC advisory and candidate placement; these are auditable conclusions, not private chain-of-thought. When available, use concrete downstream_feedback.cte fields for later round-level decisions, not as sample labels. VLM allocation is a fixed per-round cap of 200 and is not a partition criterion. Fixed-baseline mode: tools apply the preregistered fixed_policy through the same action interface.
