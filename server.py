...
- The detector is the IROS2026 action-conditioned CAE: image and recorded steering are encoded separately and their averaged latent reconstructs the image. Its only decision score is raw PCC(image, reconstruction), in [-1,1]. After scoring, partition analysis supplies three label-free strategy-specific PCC-versus-sample-index scatter plots and a fully instantiated threshold-arbitration prior. When BC>=0.555, use the prior's mean-k*std, K-means (K=2), and KDE candidate arbitration. When BC<0.555 or unavailable, do not choose among statistical strategies: estimate the current anomaly ratio from PCC statistics, the scatter plot, and previous-round aggregate VLM feedback when available, then apply the corresponding empirical lower-tail quantile. The fixed 20% maximum plausible anomaly ratio is a hard upper bound, not a target. Do not invent arbitrary thresholds. Partition analysis is observation-only; apply the selected supported mode only after the prior and evidence have been inspected. CTE remains available for other round-level decisions, but it is not an input to this partition prior. score_and_fit needs no tunable weighting parameters.
...
- Adaptive mode: choose actions from observable evidence and provide concise rationales. For an applied partition, include distribution_shape, prior_assumptions_and_uncertainty, main_risk, and confidence in the evidence object; include candidate_comparison for statistical-candidate arbitration, or explain in it why direct anomaly-ratio mode is used. Follow the partition prior's BC gate, candidate placement when applicable, aggressive policy, and 20% upper bound; these are auditable conclusions, not private chain-of-thought. When available, use concrete downstream_feedback.cte fields for later round-level decisions, not as sample labels. VLM allocation is a fixed per-round cap of 200 and is not a partition criterion. Fixed-baseline mode: tools apply the preregistered fixed_policy through the same action interface.
