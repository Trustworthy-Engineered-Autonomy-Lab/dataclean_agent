...
- The detector is the IROS2026 action-conditioned CAE: image and recorded steering are encoded separately and their averaged latent reconstructs the image. Its only decision score is raw PCC(image, reconstruction), in [-1,1]. After scoring, partition analysis supplies three label-free strategy-specific PCC-versus-sample-index scatter plots and a fully instantiated threshold-arbitration prior. Treat that partition prior as authoritative for threshold selection: it uses mean-k*std, K-means (K=2), and KDE, a single keep/gray boundary, and a fixed 30% maximum plausible anomaly ratio. Do not invent arbitrary thresholds. Partition analysis is observation-only; apply the selected supported strategy only after the prior and candidate evidence have been inspected. CTE remains available for other round-level decisions, but it is not an input to this partition prior. score_and_fit needs no tunable weighting parameters.
...
- Adaptive mode: choose actions from observable evidence and provide concise rationales. For an applied partition, include distribution_shape, prior_assumptions_and_uncertainty, candidate_comparison, main_risk, and confidence in the evidence object. Follow the partition prior's BC gate, candidate placement, aggressive policy, and 30% upper bound; these are auditable conclusions, not private chain-of-thought. When available, use concrete downstream_feedback.cte fields for later round-level decisions, not as sample labels. VLM allocation is a fixed per-round cap of 200 and is not a partition criterion. Fixed-baseline mode: tools apply the preregistered fixed_policy through the same action interface.
