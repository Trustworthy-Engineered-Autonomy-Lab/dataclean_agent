...
- In round t, D_t is the immutable round input. The capability graph describes legal dependencies, not a default script. It can produce C_t through detector, scoring, partition, and resolution; controller training, deployment, collection, and round transition are included only when the conversational goal calls for them. Controller training uses weight 2 only for samples accepted back by VLM in the current round and weight 1 for all other samples; this weighting expires at the round boundary, and validation remains unweighted. Observation-only diagnosis, sensitivity analysis, bounded ablations, early stopping, and replanning remain valid.
- The detector is the IROS2026 action-conditioned CAE: image and recorded steering are encoded separately and their averaged latent reconstructs the image. Its only decision score is raw PCC(image, reconstruction), in [-1,1]. After scoring, partition analysis supplies four label-free strategy-specific PCC-versus-sample-index scatter plots and a fully instantiated threshold-arbitration prior. The BC (bimodality coefficient) gate is enforced, not advisory: when BC>=0.555 the partition strategy must be kmeans (K=2 midpoint); when BC<0.555 or BC is unavailable, the strategy must be mean_std_ratio, supplying an estimated_anomaly_ratio_percent that the runtime converts to a threshold via mean - norm.ppf(1-ratio/100)*std, not a directly chosen k. kde is legal only as a rare, explicitly justified fallback (evidence.kde_fallback_justification) when a stable KDE valley exists and the primary strategy for the branch clearly does not fit the visible evidence. Do not invent arbitrary thresholds. Partition analysis is observation-only; apply the selected supported strategy only after the prior and evidence have been inspected. CTE remains available for other round-level decisions, but it is not an input to this partition prior. score_and_fit needs no tunable weighting parameters.
...
- Physical evaluation always produces observable CTE feedback after transfer. There is no hidden-CTE mode. Use the returned real_cte_mean, real_cte_std, real_cte_rmse, real_cte_signed_mean, cte_samples, and historical deltas when choosing the next cleaning policy.
- Adaptive mode: choose actions from observable evidence and provide concise rationales. For an applied partition, include distribution_shape, prior_assumptions_and_uncertainty, candidate_comparison, main_risk, and confidence in the evidence object; include kde_fallback_justification only when selecting the kde fallback. Follow the partition prior's BC gate and candidate placement; the BC gate is enforced by the tool for freely-chosen adaptive decisions, and these are auditable conclusions, not private chain-of-thought. When available, use concrete downstream_feedback.cte fields for later round-level decisions, not as sample labels. VLM allocation is a fixed per-round cap of 200 and is not a partition criterion. Fixed-baseline mode: tools apply the preregistered fixed_policy through the same action interface, and preregistered fixed_policy or experimental_controls partition strategies are not subject to the BC gate (ablation baselines and threshold-ablation experiments may preregister any of mean_std, kmeans, kde, or mean_std_ratio for controlled comparison).
...
- When the evidence supports a round-level continue/stop decision, record it explicitly with assess_stopping. assess_stopping computes convergence diagnostics: CTE improvement rate, VLM recovery volume change, and threshold stability. Consider stopping when all three signals are saturated (CTE improvement < 5% for 2 consecutive rounds, recovery volume change < 5%, threshold change < 2%); this indicates the cleaning pipeline has reached a plateau and further rounds are unlikely to yield gains. Never change constraints after execution begins; a changed preregistered design requires a new task, while tactical choices inside the declared independent variables remain adaptive.
